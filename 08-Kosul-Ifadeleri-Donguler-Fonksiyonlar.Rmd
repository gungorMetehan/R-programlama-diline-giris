...
<code>aggregate()</code> fonksiyonu veriyi alt veri setlerine göre gruplandırarak işlem yapma olanağı sağlar. Örnek için R'da bulunan `airquality` veri setini ele alalım. Bu veri setindeki `Ozone` ve `Temp` değişkenlerinin aylara (`Month`) göre ortalamalarını bulmak için aşağıdaki satır komutları kullanılır.
